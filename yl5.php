...
$tudrukud = array('Luisa', 'Triin', 'Rebecca', 'Kaisa', 'Kristiine', 'Jaana', 'Johanna', 'Linda');
sort($tudrukud);
...
$tudrukud = array('Luisa', 'Triin', 'Rebecca', 'Kaisa', 'Kristiine', 'Jaana', 'Johanna', 'Linda');
    ...
    echo "Esimesed kolm nime on $tudrukud[0], $tudrukud[1] ja $tudrukud[2]. <br>";
    echo "Viimane nimi on $tudrukud[7]. <br>";

    $suvaline = array_rand($tudrukud);
    echo "Suvaline nimi on $tudrukud[$suvaline].";
*/
...
echo "Autode arv: ".count($mark)."<br>";
echo "VIN arv: ".count($vin)."<br>";
if(count($mark) == count($vin)) {
    echo 'Massiivid on ühepikkused. <br>';
}   else {
    echo 'Masiivid ei ole ühepikkused. <br>';
}
$eraldi = array_count_values($mark);
echo "Toyotade arv: ".$eraldi['Toyota']."<br>";
echo "Audide arv: ".$eraldi['Audi']."<br>";
*/
...
$firmad = array("Kimia","Mynte","Voomm","Twiyo","Layo","Talane","Gigashots","Tagchat","Quaxo","Voonyx","Kwilith","Edgepulse","Eidel","Eadel","Jaloo","Oyope","Jamia"."<br>");

var_dump($firmad);

$firma = array_pop($firmad);
echo "Eemaldati: ".$firma."<br>";
var_dump($firmad);
*/
...
$pikkus = array_map('strlen', $riigid);
echo 'Kõige pikema riigi nime märkide arv on '.max($pikkus).'.';
*/
